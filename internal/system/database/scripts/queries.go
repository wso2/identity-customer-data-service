...
var GetOrgConfigurations = map[string]string{
	"postgres": `SELECT * FROM cds_config WHERE org_handle = $1`,
}
...
var UpdateOrgConfigurations = map[string]string{
	"postgres": `WITH updated AS (
		...
		WHERE org_handle = $3
		RETURNING org_handle
	)
	INSERT INTO cds_config (org_handle, cds_enabled, initial_schema_sync_done)
	SELECT $3, $1, $2
	WHERE NOT EXISTS (SELECT 1 FROM updated)`,
}

var UpdateInitialSchemaSyncDoneConfig = map[string]string{
	"postgres": `UPDATE cds_config SET initial_schema_sync_done = $1  WHERE org_handle = $2`,
}
