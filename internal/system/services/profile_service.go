...
func (s *ProfileService) RegisterRoutes(mux *http.ServeMux, apiBasePath string) {
	...
	mux.HandleFunc(fmt.Sprintf("GET %s/profiles/initialize", apiBasePath), s.profileHandler.InitProfile)
	...
	mux.HandleFunc(fmt.Sprintf("GET %s/profiles?userID=", apiBasePath), s.profileHandler.GetProfileWithUserId)
