...
// Package wso2is implements identity_provider.Client against classic WSO2
// Identity Server. This is the original (and, until the Thunder onboarding,
// only) CDS identity-provider integration, moved here unchanged from
// internal/system/client/identity_client.go.
package wso2is
...
// Client is CDS's identity_provider.Client implementation for classic WSO2 IS.
type Client struct {
	BaseURL    string
	HTTPClient *http.Client
}

// NewClient creates a Client with a TLS/mTLS-ready HTTP client.
func NewClient(cfg config.Config) *Client {
	baseHostPort := cfg.AuthServer.Host
	if cfg.AuthServer.Port != "" {
		baseHostPort = cfg.AuthServer.Host + ":" + cfg.AuthServer.Port
	}
	log.GetLogger().Info("Creating wso2is identity provider client with base URL: " + baseHostPort)

	httpClient, err := utils.NewOutboundHTTPClient(cfg.TLS, cfg.AuthServer.Host)
	if err != nil {
...
func (c *Client) Name() string {
	return constants.IdentityProviderWSO2IS
}
...
func (c *Client) FetchToken(orgHandle string) (string, error) {
...
func (c *Client) buildTokenEndpoint(orgId, tokenEndpoint string) string {
...
func (c *Client) fetchOrganizationToken(
	orgHandle string,
...
func (c *Client) fetchClientCredentialsToken(orgId string, authCfg config.AuthServerConfig, scope string) (string, error) {
...
func (c *Client) requestToken(endpoint, clientID, clientSecret string,
...
func (c *Client) FetchApplicationIdentifier(applicationIdentifier, orgHandle string) (idpModel.ApplicationsListResponse, error) {
...
func (c *Client) IntrospectToken(token, orgHandle string) (map[string]interface{}, error) {
...
// VerifyJWT locally verifies a JWT's signature against IS's JWKS endpoint.
// Not yet wired into internal/system/authn — see docs/guides/identity-providers.md, Phase 2.
func (c *Client) VerifyJWT(tokenString string) (map[string]interface{}, error) {

	jwksEndpoint := "https://" + c.BaseURL + "/oauth2/jwks"
	...
	req, err := http.NewRequest("GET", jwksEndpoint, nil)
	if err != nil {
		return nil, err
	}
	resp, err := c.HTTPClient.Do(req)
...
func (c *Client) GetProfileSchema(orgHandle string) ([]model.ProfileSchemaAttribute, error) {
...
func (c *Client) GetAllDialects(orgHandle string) ([]map[string]interface{}, error) {
...
func (c *Client) GetClaimsByDialect(dialectID, orgId string) ([]map[string]interface{}, error) {
...
func (c *Client) GetLocalClaimsMap(orgId string) (map[string]map[string]interface{}, error) {
...
func (c *Client) GetSCIMUser(orgId, userId string) (map[string]interface{}, error) {
