...
import {
  addUnificationRule,
  deleteUnificationRule,
  getUnificationRules,
  patchUnificationRule,
} from '../../api';
import type { UnificationRule } from '../../models';
...
const EMPTY_RULE: RuleFormData = { rule_name: '', property_name: '', priority: 1, is_active: true };

export default function UnificationRulesPage() {
  const [rules, setRules] = useState<UnificationRule[]>([]);
  ...
  const openEdit = (rule: UnificationRule) => {
  ...
  const handleSave = async () => {
    try {
      if (editingId) {
        const { property_name: _p, ...patchData } = formData;
      ...
      setToast({ open: true, msg: `Failed: ${e instanceof Error ? e.message : String(e)}`, severity: 'error' });
    }
  };

  const handleToggle = async (rule: UnificationRule) => {
    try {
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={5}>
                    <EmptyState title="No rules" description="Create a unification rule to get started" />
                  </TableCell>
                </TableRow>
              ) : (
                sorted.map((r) => (
                  <TableRow key={r.rule_id} hover>
                    ...
                    <TableCell>
                      <Switch size="small" checked={r.is_active} onChange={() => handleToggle(r)} color="primary" />
                    </TableCell>
                    <TableCell align="right">
                      ...
                      <Tooltip title="Delete">
                        <IconButton size="small" color="error" onClick={() => setDeleteTarget(r.rule_id)}>
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button
            variant="contained"
            onClick={handleSave}
            disabled={!formData.rule_name || !formData.property_name}
          >
