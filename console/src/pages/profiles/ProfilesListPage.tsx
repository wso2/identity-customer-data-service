...
// Table components are not re-exported by @oxygen-ui/react — use MUI directly
import {
  Table,
  TableBody,
  TableCell,
...
import { createProfile, deleteProfile, getProfiles, getUnificationRules } from '../../api';
import type { Pagination, ProfileListItem, UnificationRule } from '../../models';
...
export default function ProfilesListPage() {
  ...
  const [createData, setCreateData] = useState<CreateData>({ user_id: '' });
  const [ruleFields, setRuleFields] = useState<UnificationRule[]>([]);
  ...
  const handleCreate = async () => {
    try {
      const body: { user_id?: string; identity_attributes: Record<string, string> } = {
        identity_attributes: {},
      };
                ...
                <div key={p.profile_id}>
                    ...
                    secondaryAction={
                      <Tooltip title="Delete profile">
                        <IconButton
                          edge="end"
                          size="small"
                          onClick={(e) => { e.stopPropagation(); setDeleteTarget(p.profile_id); }}
                        >
                  ...
                  >
                    ...
                    <ListItemText
                      ...
                      sx={{ flex: '1 1 50%' }}
                    />
                    <Box sx={{ flex: '0 0 160px', display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 2 }}>
